...
var Rot13 = new Module.Class({
    ...
    Template: 'resource:///name/ptomato/RefactoredBarnacle/card/rot13.ui',
    ...
    // Overrides View.set_label_or_hide
    set_label_or_hide(label, text) {
        ...
        label.label = GLib.markup_escape_text(processed, -1);
        label.visible = true;
    },

    // Overrides Card._create_inventory_widget
    _create_inventory_widget() {
        this._child_count = 0;
        const label = new FormattableLabel();

        this._count_set(this.model, count => {
            const translatedLabel = ngettext('%d item', '%d items', count);
            label.label = this._processText(translatedLabel.replace(/%d/, count));
            label.show();
        });
        return label;
    },
    ...
    _main_layout: function (card_width, card_height, text_fraction,
        text_constant, text_portion_attr, text_full_attr) {
        const constraints = [
            {
